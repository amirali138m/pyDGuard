...
class DependencyScanner:
    ...
    def get_pip_freeze(self) -> List[str]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Error executing 'pip freeze': {e.stderr}"
                ) from e

    def parse_pip_freeze(
        self,
        freeze_output: List[str]
    ) -> List[Dict[str, str]]:
        """
        ...
        for dep in dependencies:
            ...
            if dep_name in deprecated_packages:
                ...
                try:
                    ...
                    min_parts = list(map(
                        int, min_supported_version.split('.')))

                    if current_parts < min_parts:
                        dep_info['is_deprecated'] = True
                        dep_info['deprecation_reason'] = f'Version {
                            current_version
                            } is deprecated. Minimum supported version: {
                                min_supported_version}'

                except ValueError:
                    dep_info['has_warnings'] = True
                    dep_info['warnings'].append(
                        f'Version {current_version} cannot be parsed')

            if any(keyword in current_version.lower()
                   for keyword in [''
                   'alpha', 'beta', 'rc', 'dev', 'pre']):
                dep_info['has_warnings'] = True
                dep_info['warnings'].append(
                    'This is a development/pre-release version')

            if 'deprecated' in dep_name or 'legacy' in dep_name:
                dep_info['is_deprecated'] = True
                dep_info['deprecation_reason'
                         ] = 'Package name indicates deprecated status'
    ...
    try:
        ...
        for result in results:
            if result['is_deprecated']:
                print(f"DEPRECATED: {
                    result['name']}=={result['current_version']}")
